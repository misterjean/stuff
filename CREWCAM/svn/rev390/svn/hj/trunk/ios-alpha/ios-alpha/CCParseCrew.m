...
- (void) addVideoLocally:(id<CCVideo>) newVideo
{
    ...
    if ([ccVideos count] == 0)
    {
        [self reloadVideosInBackgroundWithBlockOrNil:^(BOOL succeeded, NSError *error) {
...
- (void) finishedUploadingVideoWithSuccess:(BOOL)successful error:(NSError *)error andVideoReference:(id<CCVideo>)video
{
    ...
    else
    {
        ...
        [self notifyListenersThatVideosHaveBeenAdded:nil andVideosHaveBeenRemoved:[[NSArray alloc] initWithObjects:[NSIndexPath indexPathForRow:0 inSection:0], nil]];
    }
