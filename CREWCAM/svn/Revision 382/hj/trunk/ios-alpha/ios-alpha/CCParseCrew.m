        ...
        [self pushObjectWithBlockOrNil:^(BOOL succeeded, NSError *error) 
         {         
             ...
             {            
                 ...
                 if ([ccUsersThatAreMembers count] < 1)
                 {
                     [self loadMembersInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
                     {
                         for (id<CCUser> user in ccUsersThatAreMembers)
                         {
                             if (![[user getObjectID] isEqualToString:[[[[CCCoreManager sharedInstance] server] currentUser] getObjectID]])
                             {
                                 [CCParseNotification createNewNotificationInBackgroundWithType:ccNewVideoNotification andTargetUser:user andSourceUser:[video getTheOwner] andTargetObject:video andTargetCrewOrNil:self andMessage:[[NSString alloc] initWithFormat:@"%@ added a new video to the crew \"%@\"!",
                                                                                                                                                                                                                                  [[[[CCCoreManager sharedInstance] server] currentUser ]getName], [self getName]]];
                             }
                         }
                     }];
...
- (void) loadUnwatchedVideoCountInBackgroundWithBlockOrNil:(CCIntResultBlock) block
{
    ...
    /* New way to get number once parse fixes its stuff 
    PFQuery *queryForViewer = [PFUser query];
    [queryForViewer whereKey:@"objectId" notEqualTo:[[[[CCCoreManager sharedInstance] server] currentUser] getUserID]];
    
    PFQuery *videosQuery = [videosInCrewRelation query];
    [videosQuery whereKey:@"viewedBy" matchesQuery:queryForViewer];
