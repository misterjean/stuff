...
{ 
    if ([[data objectForKey:@"type"] intValue] == CCInvitePush || [[data objectForKey:@"type"] intValue] == ccCrewPushNotification)
    {
...
- (void) clearAllNotificationsInBackground
{
    if ([ccNotifications count] < 1)
        return;
    ...
    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSLog(@"%d notifications...", [ccNotifications count]);
        // Delete all the objects on a background thread
        for(int notificationIndex = 0; notificationIndex < [ccNotifications count]; notificationIndex++)
        {
            NSLog(@"deleting!");
            [[ccNotifications objectAtIndex:notificationIndex] deleteObject];
        }
        
        [self handleNewCCObjects:emptyArray removedObjectIndexes:oldNotificationIndexes addedObjectIndexes:newNotificationIndexes finalArrayOfObjects:ccNotifications];
