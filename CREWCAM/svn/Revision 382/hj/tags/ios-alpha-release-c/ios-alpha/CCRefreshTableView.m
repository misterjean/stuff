...
- (void) addPullToRefreshFooter
{
    ...
    {
        refreshFooterView = [[UIView alloc] initWithFrame:CGRectMake(0,[self numberOfRowsInSection:0] * [self rowHeight], 320, REFRESH_FOOTER_HEIGHT)];
    }
    else {
        [refreshFooterView setFrame:CGRectMake(0,[self numberOfRowsInSection:0] * [self rowHeight], 320, REFRESH_FOOTER_HEIGHT)];
    }
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (isLoading)
    {
    if (scrollView.contentOffset.y + [self frame].size.height < ([self numberOfRowsInSection:0] * [self rowHeight]))
        self.contentInset = UIEdgeInsetsZero;
    else if (scrollView.contentOffset.y + [self frame].size.height >= [self numberOfRowsInSection:0] * [self rowHeight] + REFRESH_FOOTER_HEIGHT)
        self.contentInset = UIEdgeInsetsMake(0, 0, REFRESH_FOOTER_HEIGHT, 0);
    }
    else if (isDragging && scrollView.contentOffset.y + [self frame].size.height > [self numberOfRowsInSection:0] * [self rowHeight])
    {
        [UIView beginAnimations:nil context:nil];
        if (scrollView.contentOffset.y + [self frame].size.height > [self numberOfRowsInSection:0] * [self rowHeight] + REFRESH_FOOTER_HEIGHT)
        {
