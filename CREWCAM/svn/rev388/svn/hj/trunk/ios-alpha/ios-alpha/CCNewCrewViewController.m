...
- (void)inviteFriendsToCrew:(id<CCCrew>)crew
{
    if ([selectedCrewcamFriends count] > 0)
    {
        ...
        {
            [[[CCCoreManager sharedInstance] server] addNewInviteToCrewInBackground:crew forUser:[person ccUser] fromUser:[[[CCCoreManager sharedInstance] server] currentUser]];
        }
    }    
