   ...
   [user pushObjectWithBlockOrNil:^(BOOL succeeded, NSError *error) 
    {
         ...
         PFQuery *invitedPersonQuery = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects:facebookIDQuery, phoneNumberQuery, emailAddressQuery, nil]];
         ...
         [invitedPersonQuery findObjectsInBackgroundWithBlock:^(NSArray *invitedPeople, NSError *error){
              ...
              [user pushObjectWithBlockOrNil:^(BOOL succeeded, NSError *error) {
                   // Create invites to any crews they were invited to, and purge old data
                   ...
                   [[invitedPerson objectForKey:@"invitedBy"] fetchIfNeededInBackgroundWithBlock:^(PFObject *object, NSError *error) {
                        if (error)
                        {
                             [[[CCCoreManager sharedInstance] logger] logAtLogLevel:ccLogLevelError message:@"Unable to fetch invitor: %@", [error localizedDescription]];
                             block(NO, error);
                             return;
                        }
                        
                        id<CCUser> userThatInvited = [[CCParseUser alloc] initWithServerData:object];
                        ...
                        [[relatedCrews query] findObjectsInBackgroundWithBlock:^(NSArray *crewsInvitedTo, NSError *error) {                             
                             if (!error)
                             {
                                  ...
                                  for (PFObject *crew in crewsInvitedTo)
                                  {
                                       ...
                                       [CCParseInvite createNewInviteToCrewInBackground:ccCrew forUser:user fromUser:userThatInvited];                     
                                  }
