...
- (void) addCommentInBackground:(id<CCComment>) comment withBlockOrNil:(CCBooleanResultBlock) block
{
    ...
    {                
        if (!error)
        {
            ...
            NSMutableArray *channelsToNotify = [[NSMutableArray alloc] init];
            
            for (id<CCComment> comment in [self ccComments]) {
                if (![channelsToNotify containsObject:[[comment getCommenter] getUserChannel]] && ![[[comment getCommenter] getUserID] isEqualToString: [[self getTheOwner] getUserID]])
                {
                    [channelsToNotify addObject:[[comment getCommenter] getUserChannel]]; 
                    
                    [CCParseNotification createNewNotificationInBackgroundWithType:ccNewCommentNotification andTargetUser:[comment getCommenter] andSourceUser:[comment getCommenter] andTargetObject:self andTargetCrewOrNil:nil andMessage:[[NSString alloc] initWithFormat:@"%@ added a new comment to the video \"%@\"!",
            ...
            [[[CCCoreManager sharedInstance] server] sendNotificationWithData:messageData ToChannels:channelsToNotify];
            ...
            if (![[[comment getCommenter] getUserID] isEqualToString: [[self getTheOwner] getUserID]])
            {
                ...
                [CCParseNotification createNewNotificationInBackgroundWithType:ccNewCommentNotification andTargetUser:[self getTheOwner] andSourceUser:[comment getCommenter] andTargetObject:self andTargetCrewOrNil:nil andMessage:[[NSString alloc] initWithFormat:@"%@ added a new comment to your video \"%@\"!",
                                                                                                                                                                                                                  [[[[CCCoreManager sharedInstance] server] currentUser] getName], [self getName] ]];
            }
        }
        else
