...
@synthesize isLinkingUserToFBAccount;
        ...
        [authenticator authenticateInBackgroundWithBlock:^(id<CCUser> user, BOOL succeeded, NSError *error) {
            @try {
                if (succeeded)
                {
                    ...
                    if (forceFacebook)
                    {
                        //User is new, and this is not a background fb login attempt. Make sure an email account doesn't already exist for this user.
...
- (void) changeUsernameAndPasswordWithEmail:(NSString *)email password:(NSString *)password block:(CCBooleanResultBlock)block
{
    ...
    {
        CCCrewcamAlertView *alert = [[CCCrewcamAlertView alloc] initWithTitle:@"New Email" message:@"Enter an email that will be used as the Username for your account." withTextField:YES delegate:self cancelButtonTitle:nil otherButtonTitles:@"Submit", nil];

        [alert setTag:changeUsernamePassword];
        
        if (block)
        {
            changeUsernameAndPasswordBlockPointerHolder = block;
            changeUsernameAndPasswordPasswordHolder = password;
        }
        [alert show];
...
{
    if ([alertView tag] == changeUsernamePassword)
    {
        NSString *potentialEmail = [[alertView getTextField] text];
        
        if ([self validateEmail:potentialEmail])
        {
            [[PFUser currentUser] setUsername:potentialEmail];
            [currentUser setEmailAddress:potentialEmail];
            [[PFUser currentUser] setPassword:changeUsernameAndPasswordPasswordHolder];
            [[PFUser currentUser] saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                if (changeUsernameAndPasswordBlockPointerHolder)
                {
                    changeUsernameAndPasswordBlockPointerHolder(succeeded, error);
                }
            }];
