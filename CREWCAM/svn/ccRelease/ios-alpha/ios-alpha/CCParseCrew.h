...
@interface CCParseCrew : CCParseObject <CCCrew, CCVideoUpdatesDelegate>
{
    uint32_t isLoadingVideos;
    uint32_t isLoadingMembers;
    uint32_t isLoadingInvites;
    uint32_t isLoadingNumberOfUnwatchedVideos;
    uint32_t isLoadingNumberOfVideosFromAutoCrew;
    uint32_t isLoadingNumberOfMembersFromAutoCrew;
    
    BOOL                    didAttemptToLoadThumbnail;
    
    id<CCVideo>     videoBeingUploaded;
    UIImage         *crewThumbnail;
}
