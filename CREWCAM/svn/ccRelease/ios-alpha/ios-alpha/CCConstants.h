...
#define RECORD_TAB_BAR_INDEX                2
#define INVITES_TAB_BAR_INDEX               3
#define NOTIFICATIONS_TAB_BAR_INDEX         4
...
@protocol CCFriendRequest;

typedef void (^CCBooleanResultBlock)(BOOL succeeded, NSError *error);
typedef void (^CCFriendRequestResultBlock)(id<CCFriendRequest> request, NSError *error);
typedef void (^CCImageResultBlock)(UIImage *image, NSError *error);
...
typedef enum {
    ...
    ccInvitePushNotification,
    ccCommentPushNotification,
    ccViewPushNotification,
    ccMemberPushNotification,
    ccFriendJoinedPushNotification,
    ccFriendRequestPushNotification,
    ccFriendRequestAcceptedPushNotification,
...
//Error codes used for handling failed email login/signup cases.
typedef enum {
    ccGeneralFailure,
    ccNoNetworkConnection,
    ccPasswordsDontMatch,
    ccInvalidEmail,
    ccNoPasswordEntered,
    ccEmailInUseEmailAccount,
    ccEmailInUseFacebookAccount,
    ccEmailDoesNotExist,
    ccPasswordRecoveryInvalidEmail
} ccFailedEmailAuthenticationErrorCodes;

//Link/Unlink facebook error cases.
typedef enum {
    ccFacebookAccountAlreadyLinked,
    ccGeneralFBLinkingError,
} ccLinkUnlinkFacebookErrorCodes;

//Facebook login error cases.
typedef enum {
    ccEmailAccountAlreadyExistsForAccount,
    ccMoreThenOneAccountWithSameEmail,
    ccGeneralFacebookLoginError,
} ccFacebookLoginErrorCodes;
...
#define CC_START_SHAKING_CREWS_NOTIFICATION         @"ccStartShakingCrews"
#define CC_STOP_SHAKING_CREWS_NOTIFICATION          @"ccStopShakingCrews"
#define CC_STOP_PLAYING_VIDEOS_IN_CREW              @"ccStopPlayingVideosInCrew"
#define CC_CLICKED_RECORD_TAB_BAR_ITEM              @"ccClickedRecordTabBar"
