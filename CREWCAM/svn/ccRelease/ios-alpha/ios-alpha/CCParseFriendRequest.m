    ...
    if ([[currentUser getObjectID] isEqualToString:[[self getCCUserThatIsRequestee] getObjectID]])
    {
        ...
        [currentUser addFriendInBackground:[self getCCUserThatRequested] withBlockOrNil:^(BOOL succeeded, NSError *error) {                        
            if (error)
            {
                ...
                [[[CCCoreManager sharedInstance] logger] logAtLogLevel:ccLogLevelError message:@"Error accepting friend request: %@", [error localizedDescription]];
                
                [[CCCoreManager sharedInstance] recordMetricEvent:CC_FAILED_ACCEPTING_FRIEND_REQUEST withProperties:nil];
            ...
            [[CCCoreManager sharedInstance] recordMetricEvent:CC_ACCEPTED_FRIEND_REQUEST withProperties:nil];
