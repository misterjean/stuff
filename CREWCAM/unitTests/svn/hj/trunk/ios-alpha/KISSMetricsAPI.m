...
- (void) initializeAPIWithKey:(NSString *)apiKey
{
    ...
    if(!self.lastIdentity) //If there's no identity, generate a UUID as a temp.
    {
        ...
        CFRelease(theUUID);
        self.lastIdentity = (__bridge_transfer NSString *)string;
...
- (NSString *)urlEncode:(NSString *)prior
{
    NSString * after = (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes( NULL,(__bridge_retained  CFStringRef)prior, NULL,(CFStringRef)@"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8 );
    return after;
