...
+ (void) createNewInviteToCrewInBackground:(id<CCCrew>) crew forUser:(id<CCUser>) user fromUser:(id<CCUser>) invitor withNotification:(BOOL)sendNotification
{
    ...
    [newCCInvite pushObjectWithBlockOrNil:^(BOOL succeeded, NSError *error) 
    {
        ...
        else
        {
            if (sendNotification)
            {
                ...
                NSString *newVideoMessage = [[NSString alloc] initWithFormat:@"%@ invited you to the crew \"%@\"!",[invitor getName], [[newCCInvite getCrewInvitedTo] getName]];
