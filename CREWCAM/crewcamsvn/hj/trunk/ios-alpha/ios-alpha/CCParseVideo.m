...
- (void) addViewInBackgroundInnerInner:(id<CCUser>) viewer video:(id<CCVideo>)videoToPush withBlockOrNil:(CCBooleanResultBlock)block
{
...
- (void) addViewInBackgroundInner:(id<CCUser>) viewer withBlockOrNil:(CCBooleanResultBlock) block
{
            ...
            for (id<CCCrew> crew in [[[[CCCoreManager sharedInstance] server] currentUser] ccCrews] )
            {
                ...
                [crewVideosToTestQuery findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                   if ([objects count] == 1)
                   {
                       ...
                       [self addViewInBackgroundInnerInner:viewer video:videoToPush withBlockOrNil:[block copy]];
                   }
                }];
            }
        }];
    ...
    {
        [self addViewInBackgroundInnerInner:viewer video:self withBlockOrNil:[block copy]];
    }
}
