...
- (void) handleNewNotifications:(NSArray *) pfObjects
{
    ...
    NSString *groupedNotificationText = [[NSString alloc] init];
    NSString *friendJoinedText = [[NSString alloc] init];
    
    for (CCParseNotification *notification in allNotifications)
    {

        for (CCParseNotification *secondNotification in allNotifications)
        {
            if ([[notification getTargetObjectId] isEqualToString:[secondNotification getTargetObjectId]]
                && [notification getNotificationType] == [secondNotification getNotificationType])
            {
                switch ([notification getNotificationType]) {
                    case ccNewCommentNotification:
                    {
                        if ([groupedNotificationText rangeOfString:[[notification getSourceUser] getName]].location == NSNotFound)
                        {
                        groupedNotificationText = [groupedNotificationText stringByAppendingString:[[notification getSourceUser] getName]];
                        }
                        break;
                    }
                    case ccFriendJoinedNotification:
                    {
                        if ([friendJoinedText rangeOfString:[[notification getSourceUser] getName]].location == NSNotFound)
                        {
                            friendJoinedText = [friendJoinedText stringByAppendingString:[[notification getSourceUser] getName]];
                        }
                        break;
                    }
        ...
        NSLog(@"good text?? %@", groupedNotificationText);
        NSLog(@"try %@", friendJoinedText);

}
    
    for(int notificationIndex = 10; notificationIndex < [allNotifications count];)
    {
    ...
    [self handleNewCCObjects:allNotifications removedObjectIndexes:oldNotificationIndexes addedObjectIndexes:newNotificationIndexes finalArrayOfObjects:ccNotifications];
