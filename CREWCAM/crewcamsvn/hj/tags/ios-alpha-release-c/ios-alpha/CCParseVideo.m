...
//This if block is not used. It should be used if we ever decide to do our own compression.
...
#endif
        PFFile *videoFile = [PFFile fileWithName:[NSString stringWithFormat:@"%@.MOV", [self alphaNumbericStringWithString:name]] contentsAtPath:videoPath];
        ...
        newCCVideo->videoFileForUpload = videoFile;
    ...
    @catch (NSException *exception) 
    {
        ...
        [delegate finishedUploadingVideoWithSuccess:NO error:error andVideoReference:nil];
        block(nil, NO, error);
    }
}
...
- (void) uploadAndSaveInBackgroundWithBlock:(CCVideoResultBlock) block
{
    ...
    videoProgressTimer = [NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(videoUploadTimeout:) userInfo:block repeats:NO];
    [self notifyListenersThatUploadIsStarting];
    
    [videoFileForUpload saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) 
     {                          
         if (error) 
         {
             ...
             [videoImageFile saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) 
              {
                  ...
                  {                     
                      [[self parseObject] setObject:videoFileForUpload forKey:@"videoFile"];                     
...
{
    CCVideoResultBlock block = [timer userInfo];
...
{
    if ([CCParseObject isObjectInArray:viewer arrayOfCCServerStoredObjects:ccUsersThatViewed])
        return;
...
- (void) addCommentInBackground:(id<CCComment>) comment withBlockOrNil:(CCBooleanResultBlock) block
{
    ...
    {                
        if (!error)
        {
            ...
            NSMutableArray *usersToNotify = [[NSMutableArray alloc] init];
            
            for (id<CCComment> comment in [self ccComments]) {
                if (![usersToNotify containsObject:[[comment getCommenter] getUserChannel]] && ![[[comment getCommenter] getUserID] isEqualToString: [[self getTheOwner] getUserID]])
                {
                    [usersToNotify addObject:[[comment getCommenter] getUserChannel]]; 
                }
            ...
            [[[CCCoreManager sharedInstance] server] sendNotificationWithData:messageData ToChannels:usersToNotify];
            
            if (![[[comment getCommenter] getUserID] isEqualToString: [[self getTheOwner] getUserID]])
            {
                ...
                [[self getTheOwner] sendNotificationWithData:messageData];
            }
        }
        else
        {
