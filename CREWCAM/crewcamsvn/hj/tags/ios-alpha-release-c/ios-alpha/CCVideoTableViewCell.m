...
- (void)initializeWithVideo:(id<CCVideo>) videoForCell andNavigationController:(UINavigationController *) navigationController
{
    ...
    else 
    {
        ...
        [timeSinceVideoPostLabel setText:[self getTimeSinceString]];
...
{
    NSTimeInterval timeSincePosting = [[NSDate date] timeIntervalSinceDate:[video getObjectCreatedDate]];
    
    NSString *timeSinceString;
    
    if (timeSincePosting/60 < 1) 
    {
        timeSinceString = [[NSString alloc] initWithFormat:@"%.f seconds ago", timeSincePosting];    
    }
